...
const vapidSubject = process.env.VAPID_SUBJECT || "mailto:enesimrakresmi@gmail.com";
...
export async function sendPushNotification({ targetUser, senderUser, title, body, url = "/", tag = "efes-notification" }) {
  ...
  try {
    ...
    if (tag) {
      // APNs topic/collapse identifier
      pushOptions.topic = String(tag).slice(0, 32);
    }
    ...
    const results = await Promise.allSettled(
      subs.map(async (sub) => {
        try {
          const res = await webpush.sendNotification(sub.subscription, payload, pushOptions);
          ...
          console.error(`Push send failed for [${sub.user_alias}] (${sub.id}): status=${err.statusCode} msg=${err.message}`);
    ...
    return { count: subs.length, delivered: successful, cleaned: deadSubIds.length };
  } catch (err) {
